...
import com.danikula.videocache.bandwidth.DownloadLimitInputStream;
...
public class HttpUrlSource implements Source {
    ...
    public HttpUrlSource(String url, Config config) {
        this(url, SourceInfoStorageFactory.newEmptySourceInfoStorage(), config);
    }

    public HttpUrlSource(String url, SourceInfoStorage sourceInfoStorage, Config config) {
        this(url, sourceInfoStorage, new EmptyHeadersInjector(), config);
    }

    public HttpUrlSource(String url, SourceInfoStorage sourceInfoStorage, HeaderInjector headerInjector, Config config) {
    ...
    public boolean open(long offset) throws ProxyCacheException {
        ...
        try {
            ...
            int singleFileBandwidth=0;
            if (config!=null){
                singleFileBandwidth=config.getSingleFileBandwidth();
            }
            DownloadLimitInputStream downloadLimitInputStream = new DownloadLimitInputStream(connection.getInputStream(), new BandwidthLimiter(singleFileBandwidth));
            inputStream = new BufferedInputStream(downloadLimitInputStream, DEFAULT_BUFFER_SIZE);
